...
func MulModMont_11Limbs(out_bytes, x_bytes, y_bytes *[]byte, modCtx *ModContext) error {
	x := (*[11]uint64)(unsafe.Pointer(&(*x_bytes)[0]))[:]
	y := (*[11]uint64)(unsafe.Pointer(&(*y_bytes)[0]))[:]
	out := (*[11]uint64)(unsafe.Pointer(&(*out_bytes)[0]))[:]
	mod := (*[11]uint64)(unsafe.Pointer(&modCtx.Modulus[0]))[:]

	return _MulModMont_11Limbs(out, x, y, mod, modCtx.MontParamInterleaved)
}

// NOTE: len(z) == len(x) == len(y) == len(mod) && x,y < mod assumed
func _MulModMont_11Limbs(z, x, y, mod []uint64, modinv uint64) error {
	var t [11]uint64
	var c [11]uint64
