...
func MulModMont_10Limbs(out_bytes, x_bytes, y_bytes *[]byte, modCtx *ModContext) error {
	x := (*[10]uint64)(unsafe.Pointer(&(*x_bytes)[0]))[:]
	y := (*[10]uint64)(unsafe.Pointer(&(*y_bytes)[0]))[:]
	out := (*[10]uint64)(unsafe.Pointer(&(*out_bytes)[0]))[:]
	mod := (*[10]uint64)(unsafe.Pointer(&modCtx.Modulus[0]))[:]

	return _MulModMont_10Limbs(out, x, y, mod, modCtx.MontParamInterleaved)
}

// NOTE: len(z) == len(x) == len(y) == len(mod) && x,y < mod assumed
func _MulModMont_10Limbs(z, x, y, mod []uint64, modinv uint64) error {
	var t [10]uint64
	var c [10]uint64
